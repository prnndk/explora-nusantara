        ...
        init() {
            window.addEventListener('newMessageReceived', () => {
                if (this.slideOverOpen) {
                    this.$wire.markAsRead();
                }
            });
        }
    }">
       <button
            @click="slideOverOpen = true; $wire.markAsRead()"
            class="relative inline-flex items-center justify-center h-10 px-4 py-2 text-sm font-medium transition-colors bg-white border rounded-md hover:bg-neutral-100 active:bg-white focus:bg-white focus:outline-none focus:ring-2 focus:ring-neutral-200/60 focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none">
            <x-heroicon-s-chat-bubble-left-right class="size-4 mx-2" />
            <span class="ml-2" wire:poll.5s="updateChats">Mulai Percakapan</span>
            @if($this->unreadChatsCount > 0)
                <span class="absolute -top-2 -right-2 flex items-center justify-center min-w-5 h-5 px-1.5 text-xs text-white bg-red-500 rounded-full z-10">
                                ...
                                <div
                                    ...
                                    <div class="relative flex-1 px-4 mt-5 sm:px-5">
                                        <div class="absolute inset-0 px-4 sm:px-5">
                                            <div
                                                class="relative h-full overflow-y-scroll overflow-x-hidden p-3 pb-20 border border-dashed rounded-md border-neutral-300"
                                                ...
                                                <div class="flex w-full flex-col gap-4">
                                                    ...
                                                    <div
                                                        ...
                                                        <span
                                                            class="ml-auto text-xs">{{ $chat->created_at->format('h:i A') }}@if($chat->read_status)
                                                            | Seen
                                                            @endif </span>
                                                    </div>
                                                    @else
                                                    <div class="mr-auto flex max-w-[80%] flex-col gap-2 rounded-2xl rounded-tl-none bg-neutral-50 p-4 text-neutral-900 md:max-w-[60%] dark:bg-neutral-900 dark:text-white">
                                                        <span class="font-semibold flex items-center gap-2"> {{-- Tambah flex & gap --}}
                                                            ...
                                                            {{-- ✅ TAMBAHKAN INI: Bulatan merah jika belum dibaca --}}
                                                            @if(!$chat->read_status)
                                                        ...
                                                        <span class="ml-auto text-xs">{{ $chat->created_at->format('h:i A') }}</span>
                                                    </div>
                                                    @endif
                                                    @empty
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (sendMessageInput) {
            sendMessageInput.addEventListener('input', () => {
                // Pastikan tanda panah rapat: $transactionChats->last()
                const latestMessage = @js($transactionChats->last());
                if (latestMessage && latestMessage.read_status === false) {
                    Livewire.dispatch('updateChats', {
