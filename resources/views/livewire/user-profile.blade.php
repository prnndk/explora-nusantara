<div>
    ...
    <div class="grid md:grid-cols-2 gap-4 mt-8 min-h-full">
        <div class="flex flex-col items-center gap-4">
            @php
            $photo = $user->buyer?->photoFile?->id
            ?? $user->seller?->photoFile?->id
            ?? $user->photo_file_id;
            @endphp

            <div class="w-48 h-48 rounded-full overflow-hidden">
                <img
                    src="{{ $photo ? '/view-file/'.$photo : 'https://ui-avatars.com/api/?name='.$user->username }}"
        ...
        <div class="flex flex-col gap-4" x-data="{ tab: 'detail' }">
            ...
            <div class="grid grid-cols-1 gap-4" x-show="tab === 'account'">
                ...
                <x-input.select label="Bank" name="bank_name" wire:model="bank_name" :required="true">
                    <option value="">Select Bank</option>
                    @foreach(['BCA','BRI','BNI','Mandiri','CIMB Niaga','BTN','BSI'] as $bank)
