    ...
    <h6>Easily manage your account information and preferences in one intuitive and personalized place.
        ...
        <div class="grid md:grid-cols-2 gap-4 mt-8 min-h-full">
            <div class="flex flex-col items-center gap-4">
                ...
                @else
                    <img src="/view-file/{{$user->buyer->photoFile->id ?? $user->seller->photoFile->id}}"
                         alt="User"
                         class="w-48 h-48 rounded-full transition-all"/>
                @endif
                ...
                <div class="flex flex-col gap-4" x-data="{ tab: 'detail' }">
                    ...
                    <div class="grid grid-cols-1 gap-4" x-show="tab === 'detail'">
                        <x-input.text label="Your Name" name="name"
                                      value="{{$user->buyer->name ?? $user->seller->name}}"
                                      :required="true" wire:model="name"/>
                        ...
                        <x-input.tel label="Phone Number" name="phone_number"
                                     value="{{$user->buyer->phone_number ?? $user->seller->phone_number}}"
                                     :required="true" wire:model="phone_number"/>
                            ...
                            <div x-data="{ dropdownOpen: false }" class="relative">
                                ...
                                <div x-show="dropdownOpen" @click.away="dropdownOpen = false"
                                    ...
                                    <div
                                        ...
                                        <a href="/view-file/{{$user->buyer->ktpFile->id ?? $user->seller->ktpFile->id}}" @click="dropdownOpen = false"
                    ...
                    <div class="grid grid-cols-1 gap-4" x-show="tab === 'account'">
                        ...
                        <x-input.text label="Bank" name="bank_name"
                                      value="{{$user->buyer->bank_name ?? $user->seller->bank_name}}"
                                      wire:model="bank_name" :required="true"/>
                    ...
                    <div class="grid grid-cols-1 gap-4" x-show="tab === 'company'">
                        <x-input.text label="Company Name" name="company_name"
                                      ...
                                      :required="true"/>
                        @if($user->isSeller())
                            <x-input.text label="NPWP" name="npwp" value="{{$user->seller->npwp}}" wire:model="npwp"
                                          :required="true"/>
                            <x-input.text label="NIB" name="nib" value="{{$user->seller->nib}}" wire:model="nib"
                                          :required="true"/>
                        @else
                            <x-input.text label="Country" name="country"
                                          value="{{$user->buyer->country ?? $user->seller->country}}"
                                          wire:model="country" :required="true"/>
                        @endif
                        <x-input.tel label="Company Phone Number" name="company_phone_number"
                                     ...
                                     :required="true"/>
                        <x-input.text label="Company Address" name="company_address"
                        ...
                        @if($user->isSeller())
                            <div>
                            <span
                                class="block mb-2 text-sm tracking-wide text-gray-500">Scan Surat Rekomendasi Dinas Provinsi</span>
                                <div x-data="{ dropdownOpen: false }" class="relative">

                                    <button @click="dropdownOpen = true"
                                            class="inline-flex items-center justify-center h-10 px-4 py-2 text-sm font-medium transition-colors bg-white border rounded-md hover:bg-neutral-100 active:bg-white focus:bg-white focus:outline-none focus:ring-2 focus:ring-neutral-200/60 focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none">
                                        <x-heroicon-o-document class="w-5 h-5 mr-3"/>
                                        {{str_replace('uploads/', '', $user->seller->recommendationLetterFile->file_path) ?? 'file.pdf'}}
                                        <x-heroicon-c-ellipsis-vertical
                                            class="w-5 h-5 ml-3"/>
                                    </button>
                        ...
                        @else
                            <div>
                            <span
                                class="block mb-2 text-sm tracking-wide text-gray-500">Scan Surat Legalitas Usaha</span>
                                <div x-data="{ dropdownOpen: false }" class="relative">
