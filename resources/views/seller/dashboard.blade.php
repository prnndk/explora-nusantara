<x-layouts.dashboard webTitle="Dashboard Seller">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
        <div class="flex flex-col h-full w-full col-span-2">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12"
                ...
                <x-dashboard-statistic title="Verified Products" :heading="$unvalidatedProduct" :smallNumber="$totalProduct"
                ...
                <div class="flex flex-col my-4 gap-4">
                    @foreach ($newestMeeting as $meet)
                        <x-dashboard-notification heading="Zoom Meeting" description="With {{ $meet->buyer->name }}"
                            icon="heroicon-o-chat-bubble-left-right" color="secondary" :link="route('seller.trade-meeting.index')" />
                    @endforeach
                    ...
                    @foreach ($newestTransaction as $transaction)
                        <x-dashboard-notification heading="Product {{ $transaction->status }}"
                            ...
                            icon="heroicon-o-archive-box" :link="route('seller.transaction.detail', $transaction->id)" />
                    @endforeach
                </div>
            </div>
        </div>
