    ...
    <div class="fixed top-0 left-0 h-screen bg-mainGreen text-white transition-all duration-300"
        ...
        <div class="flex items-center px-4 py-3">
            ...
            <div class="ml-3 transition-all" :class="{ 'block': sidebarOpenStatus, 'hidden': !sidebarOpenStatus }">
                <h2 class="text-base font-semibold">{{ $userName ?? 'Seller' }}</h2>
                <p class="text-sm text-gray-300">{{ $userEmail ?? 'seller@gmail.com' }}</p>
            </div>
        </div>
        ...
        <nav class="mt-3">
            <ul>
                <x-layouts.sidebar.sidebar-item route="{{ route('testing') }}" :title="'Dashboard'" :active="request()->routeIs('sadadsad')"
                ...
                <x-layouts.sidebar.item-dropdown icon="heroicon-s-clipboard-document-check" title="Validation">
                    <x-layouts.sidebar.sidebar-item route="{{ route('testing') }}" :title="'Product'" :active="request()->routeIs('testing')"
                        icon="heroicon-s-shopping-cart" :isSubmenu="true" />
                    <x-layouts.sidebar.sidebar-item route="{{ route('testing') }}" :title="'Transaction'" :active="request()->routeIs('gading')"
        ...
        <div class="h-max-screen flex items-end justify-center py-6 my-8">
            <button class="flex items-center px-6 py-3 text-gray-300 hover:bg-ecstasy rounded-lg">
