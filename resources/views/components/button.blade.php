@php
    $styles = [
        ...
        'success' =>
            'text-white transition-colors duration-200 rounded-md bg-teal-600 focus:ring-2 focus:ring-offset-2 focus:ring-ecstasy focus:shadow-outline focus:outline-none',
        'outline-neutral' =>
...
@php
    $loadingTarget = $attributes->get('wire:target')
        ?? $attributes->get('wire:target.prevent')
...
<div>
    <button type="{{ $button }}" {{ $isDisabled ? 'disabled' : '' }}
        ...
        @if ($loadingTarget)
            <div role="status" class="mr-3" wire:loading wire:target="{{ $loadingTarget }}">
